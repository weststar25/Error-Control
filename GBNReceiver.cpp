...
#pragma comment(lib,"ws2_32.lib")   //소켓
#define PORT 3000
#define BLACK 0
#define LIGHTGRAY 7
#define LIGHTRED 12 
...
void packet(header (&frame)[100], int (&timeout)[30], SOCKET &C_Socket, int (&cnt)[4])
{
	...
	{      
		...
		while(cnt[1] < cnt[0])   //ACK 또는 NAK 전송
		{
			...
			else if(flag == 1)   //이전 frame중 loss나 timeout발생으로 인해 재전송을 요청한 경우, 무조건 frame을 재전송 요청
			{
				recv(C_Socket, (char *)&frame[cnt[1]], sizeof(frame[cnt[1]]), 0);
				printf("frame 수신 (GBN) → seq number : %d번째, Data : %s\n", frame[cnt[1]].Seq_num, frame[cnt[1]].Data);
